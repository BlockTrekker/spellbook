...
bebop_raw_data AS (
    ...
    LEFT JOIN {{ source('bebop_v3_ethereum', 'BebopAggregationContract_call_SettleAggregateOrder') }} ON ex.call_tx_hash = evt_tx_hash
    WHERE
        call_success = true
...
LEFT JOIN {{ ref('tokens_erc20') }}
    ON
        cast(t_bought.contract_address AS string) = t.maker_token
        AND t_bought.blockchain = 'ethereum'
LEFT JOIN {{ ref('tokens_erc20') }}
    ON
        cast(t_sold.contract_address AS string) = t.taker_token
        AND t_sold.blockchain = 'ethereum'
LEFT JOIN {{ source('prices', 'usd') }}
    ON
        p_bought.minute = date_trunc('minute', t.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }}
    ON
        p_sold.minute = date_trunc('minute', t.block_time)
