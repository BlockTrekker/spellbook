...
batch_counts AS (
    SELECT
        try_cast(date_trunc('day', s.evt_block_time) AS date) AS block_date,
        s.evt_block_time,
        s.evt_tx_hash,
        solver,
        name,
    ...
    LEFT OUTER JOIN {{ source('gnosis_protocol_v2_gnosis', 'GPv2Settlement_evt_Interaction') }}
        ON
            i.evt_tx_hash = s.evt_tx_hash
            {% if is_incremental() %}
                AND i.evt_block_time >= date_trunc('day', now() - interval '1 week')
            {% endif %}
    INNER JOIN cow_protocol_gnosis.solvers
        ON solver = address
    {% if is_incremental() %}
...
batch_values AS (
    ...
    LEFT OUTER JOIN {{ source('prices', 'usd') }}
        ON
            p.contract_address = '0xe91d153e0b41518a2ce8dd3d7944fa863463a97d'
...
combined_batch_info AS (
    SELECT
        ...
        token_approvals
    FROM batch_counts
    INNER JOIN batch_values
        ON b.evt_tx_hash = t.tx_hash
    INNER JOIN {{ source('gnosis', 'transactions') }}
        ON evt_tx_hash
= hash
            {% if is_incremental() %}
