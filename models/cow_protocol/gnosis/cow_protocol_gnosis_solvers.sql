...
known_solver_metadata (address, environment, name) AS (
    SELECT *
    FROM (
        VALUES ('0xd8da60bDe22461D7Aa11540C338dC56a0E546b0D', 'barn', 'Legacy'),
...
SELECT
    CAST(solver AS VARCHAR(42)) AS address,
    COALESCE(environment, 'new') AS environment,
    COALESCE(name, 'Uncatalogued') AS name,
    active
