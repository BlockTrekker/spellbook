...
batch_counts AS (
    ...
    FROM {{ source('gnosis_protocol_v2_ethereum', 'GPv2Settlement_evt_Settlement') }}
    LEFT OUTER JOIN {{ source('gnosis_protocol_v2_ethereum', 'GPv2Settlement_evt_Interaction') }}
        ON
...
batch_values AS (
    ...
    LEFT OUTER JOIN {{ source('prices', 'usd') }}
        ON
            p.contract_address = '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
