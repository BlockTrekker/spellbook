...
bid_sales as (
    select
        ...
        a.evt_block_time,
        a.evt_tx_hash
    from cryptopunks_bids_and_sales

    inner join {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_Transfer') }}
        on a.from_address = b.from and a.evt_block_number = b.evt_block_number and a.evt_index = (b.evt_index + 1)

    left outer join cryptopunks_bids_and_sales
...
regular_sales as (
    select
        ...
        a.evt_tx_hash
    from {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_PunkBought') }}
    left outer join {{ source('cryptopunks_ethereum','CryptoPunksMarket_evt_PunkTransfer') }}
        on
...
left join {{ ref('nft_ethereum_aggregators') }} on agg.contract_address = tx.to
