...
SELECT
    lower(trim(address)) AS address,
    trim(entity) AS entity,
    trim(entity_unique_name) AS entity_unique_name,
    trim(category) AS `category`
FROM
    (
        VALUES
...
FROM (
    ...
    INNER JOIN {{ source('ethereum', 'traces') }}
        ON
            et2.from = et.from
...
INNER JOIN {{ source('ethereum', 'traces') }}
    ON
        ...
        {% endif %}
INNER JOIN contracts ON c.address = txs.to
WHERE
    txs.to IN (SELECT address FROM contracts)
    {% if not is_incremental() %}
