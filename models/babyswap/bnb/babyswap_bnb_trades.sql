...
WITH babyswap_dex AS (
    SELECT t.evt_block_time AS block_time,
        to                                                           AS taker,
            ...
            ''                                                           AS trace_address,
            t.evt_index
    FROM {{ source('babyswap_bnb', 'BabyPair_evt_Swap') }}
    INNER JOIN {{ source('babyswap_bnb', 'BabyFactory_evt_PairCreated') }}
...
LEFT JOIN {{ ref('tokens_erc20') }}
    ON
        erc20a.contract_address = babyswap_dex.token_bought_address
        AND erc20a.blockchain = "bnb"
LEFT JOIN {{ ref('tokens_erc20') }}
    ON
        erc20b.contract_address = babyswap_dex.token_sold_address
        AND erc20b.blockchain = "bnb"
LEFT JOIN {{ source('prices', 'usd') }}
    ON
        p_bought.minute = date_trunc("minute", babyswap_dex.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }}
    ON
        p_sold.minute = date_trunc("minute", babyswap_dex.block_time)
