...
WITH mints AS (
    SELECT
        tr.contract_address AS project_contract_address,
        ...
        CASE
            WHEN cardinality(_royaltyamounts) > 1 THEN CAST(10 AS DOUBLE)
            WHEN cardinality(_royaltyamounts) = 1 THEN CAST(element_at(_royaltyamounts, 1) AS DOUBLE)
            ELSE CAST(0 AS DOUBLE)
        END AS `royalty_fee_percentage`

    FROM {{ source('nftb_bnb', 'NFT_evt_Transfer') }}

    INNER JOIN {{ source('nftb_bnb', 'NFT_evt_Mint') }}
        ON tr.evt_tx_hash = m.evt_tx_hash
    WHERE
        tr.evt_block_time >= '{{ project_start_date }}'
        {% if is_incremental() %}
...
trades AS (
    SELECT
        ...
        tr.to AS buyer,
        CASE
            WHEN tr.from = '0xdc2f08364ebc6cebe0b487fc47823b1e83ce8550' --- claim contract address
        ...
        CASE
            WHEN tr.from = '0xebd4232e4c1999bc9562802eae01b431d5053e65'
        ...
        CASE
            WHEN tr1.value = 0 AND tra1.address = '0xebd4232e4c1999bc9562802eae01b431d5053e65' --seller contract address with withdraw function is called (auction settled)
                THEN tra1.value
...
{% endif %}
    ...
    LEFT JOIN (
        ...
        WHERE
            1 = 1
            AND (
                bt.from = '0xebd4232e4c1999bc9562802eae01b431d5053e65' --seller contract address when withdraw function is called
                OR
    ...
    LEFT JOIN {{ source('bnb','logs') }}
        ON
            l1.tx_hash = r.call_tx_hash
...
LEFT JOIN {{ source('erc721_ethereum','evt_transfer') }}
    ON
        erc721.evt_block_time = ae.block_time
...
LEFT JOIN {{ ref('tokens_bnb_nft') }}
    ON nft_token.contract_address = ae.nft_contract_address

LEFT JOIN {{ ref('nft_bnb_aggregators') }}
