...
iv_orders_matched as (
    select
        evt_block_time as om_block_time,
...
iv_enh_base_pairs as (
    select
        ...
        b.om_evt_index,
        b.om_order_id
    from ref_seaport_ethereum_base_pairs
    left join iv_orders_matched
        on
            b.om_order_hash = a.order_hash
...
iv_trades as (
    select
        ...
        sub_idx
    from iv_nfts
    inner join source_ethereum_transactions on t.hash = a.tx_hash
    ...
    left join ref_nft_aggregators_marks on right(t.data, agg_m.hash_marker_size) = agg_m.hash_marker
)
