...
with iv_availadv as (
    ...
    from (
        ...
        from {{ source('seaport_ethereum','Seaport_call_fulfillAvailableAdvancedOrders') }}
        where call_success
    )
),
...
iv_transfer_level_pre as (
    ...
    inner join (
        ...
        as `e` on a.recipient = e.recipient
...
iv_transfer_level as (
    select a.*
    from iv_transfer_level_pre
    left join iv_availadv
        on
            b.tx_hash = a.tx_hash
    ...
    left join {{ source('seaport_ethereum','Seaport_call_fulfillAvailableAdvancedOrders') }} on c.call_tx_hash = a.tx_hash
    where
        1 = 1
...
iv_txn_level as (
    select
        ...
        sum(case
            when category = 'auction' and sub_idx in (1, 2) then original_amount
        ...
        max(case
            when category = 'auction' and sub_idx in (1, 2) then token_contract_address
        ...
        max(case
            when category = 'auction' and sub_idx = 2 then receiver
        ...
        sum(case
            when category = 'auction' and sub_idx = 2 then original_amount
        ...
        max(case
            when category = 'auction' and sub_idx = 2 then token_contract_address
        ...
        max(
            case when nft_transfer_count = 1 and item_type in ('2', '3') then token_contract_address
            end
        ) as nft_contract_address,
        max(
            case when nft_transfer_count = 1 and item_type in ('2', '3') then token_id
            end
        ) as nft_token_id,
...
iv_nft_trades as (
    select
        ...
        nft_token_id,
        case
            when erc721_transfer_count > 0 and erc1155_transfer_count = 0 then 'erc721'
        ...
        case
            when a.original_currency_contract = '0x0000000000000000000000000000000000000000' then 'ETH'
            else p1.symbol
        ...
        case
            when a.fee_currency_contract = '0x0000000000000000000000000000000000000000' then 'ETH'
            else p2.symbol
        end as fee_currency,
    ...
    from iv_txn_level
    left join {{ source('ethereum','transactions') }}
        on
            tx.hash = a.tx_hash
            and tx.block_number > 14801608
    left join {{ ref('tokens_ethereum_nft') }} on n.contract_address = a.nft_contract_address
    left join {{ ref('tokens_ethereum_erc20') }} on e1.contract_address = a.currency_contract
    left join {{ ref('tokens_ethereum_erc20') }} on e2.contract_address = a.currency_contract2
    left join {{ source('prices', 'usd') }}
        on
            p1.contract_address = a.currency_contract
            and p1.minute = date_trunc('minute', a.block_time)
            and p1.minute >= '2022-05-15'
            and p1.blockchain = 'ethereum'
    left join {{ source('prices', 'usd') }}
        on
            p2.contract_address = a.currency_contract2
            and p2.minute = date_trunc('minute', a.block_time)
            and p2.minute >= '2022-05-15'
            and p2.blockchain = 'ethereum'
    left join {{ source('seaport_ethereum','Seaport_call_fulfillOrder') }} on spc1.call_tx_hash = a.tx_hash
    left join {{ source('seaport_ethereum','Seaport_call_fulfillBasicOrder') }} on spc2.call_tx_hash = a.tx_hash
    left join {{ source('seaport_ethereum','Seaport_call_fulfillAdvancedOrder') }} on spc3.call_tx_hash = a.tx_hash
    left join {{ source('seaport_ethereum','Seaport_call_fulfillAvailableAdvancedOrders') }} on spc4.call_tx_hash = a.tx_hash
    left join {{ source('seaport_ethereum','Seaport_call_fulfillAvailableOrders') }} on spc5.call_tx_hash = a.tx_hash
    left join {{ source('seaport_ethereum','Seaport_call_matchOrders') }} on spc6.call_tx_hash = a.tx_hash
)
