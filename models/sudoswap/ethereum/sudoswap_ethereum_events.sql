...
swaps_with_calldata AS (
    ...
    FROM swaps AS s
    INNER JOIN {{ source('ethereum', 'traces') }} AS tr
        ON
            tr.success AND s.call_block_number = tr.block_number AND s.call_tx_hash = tr.tx_hash AND s.call_trace_address = tr.trace_address
            {% if is_incremental() %}
                -- this filter will only be applied on an incremental run. We only want to update with new swaps.
...
swaps_w_fees AS (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM swaps_with_calldata AS swaps
            INNER JOIN pairs_created AS pc ON pc.pair_address = contract_address --remember swaps from other NFT addresses won't appear!
            ...
            LEFT JOIN pool_fee_update AS fu ON swaps.call_block_time >= fu.evt_block_time AND swaps.contract_address = fu.contract_address
            LEFT JOIN protocol_fee_update AS pfu ON swaps.call_block_time >= pfu.evt_block_time
            LEFT JOIN asset_recipient_update AS aru ON swaps.call_block_time >= aru.evt_block_time AND swaps.contract_address = aru.contract_address
        ) AS a
    ) AS b
...
swaps_w_traces AS (
    ...
    SELECT
        ...
        SUM(
            CASE
                WHEN sb.trade_category = 'Buy' -- caller buys, AMM sells
                    THEN (
                        CASE
                            WHEN tr.from = sb.call_from THEN value -- amount of ETH payed
                ...
                ELSE ( -- caller sells, AMM buys
                    CASE
                        WHEN tr.from = sb.pair_address THEN value -- all ETH leaving the pool, nothing should be coming in on a sell.
                        ELSE 0
                    END
                )
            END
        ) AS trade_price, -- what the buyer paid (incl all fees)
        SUM(
            CASE
                WHEN (tr.to = sb.protocolfee_recipient) THEN value
                ELSE 0
            END
...
swaps_cleaned AS (
    ...
    SELECT
        ...
        CASE
            WHEN cardinality(token_id) > 1 THEN 'Bundle Trade'
            ELSE 'Single Item Trade'
        END AS trade_type,
        trade_category,
        'Trade' AS evt_type,
        CASE
            WHEN trade_category = 'Buy' THEN pair_address --AMM is selling if an NFT is being bought
            ELSE trade_recipient
        END AS seller,
        CASE
            WHEN trade_category = 'Sell' THEN pair_address --AMM is buying if an NFT is being sold
            ELSE trade_recipient
        END AS buyer,
...
swaps_cleaned_w_metadata AS (
    SELECT
        ...
        tx.from AS tx_from,
        tx.to AS tx_to
    FROM swaps_cleaned AS sc
    INNER JOIN {{ source('ethereum', 'transactions') }} AS tx
        ON
            tx.block_number = sc.block_number AND tx.hash = sc.tx_hash
            {% if is_incremental() %}
    ...
    LEFT JOIN {{ ref('nft_aggregators') }} AS agg
        ON
            (agg.contract_address = sc.call_from OR agg.contract_address = sc.router_caller) -- aggregator will either call pool directly or call the router
    ...
    LEFT JOIN tokens_ethereum_nft AS tokens ON nft_contract_address = tokens.contract_address
),
...
swaps_exploded AS (
    SELECT
        ...
        token_standard,
        (number_of_items / number_of_items)::decimal(38, 0) AS number_of_items,
        trade_type,
        trade_category,
        evt_type,
        seller,
        buyer,
        (amount_raw / number_of_items)::decimal(38, 0) AS amount_raw,
        ...
        (platform_fee_amount_raw / number_of_items)::double AS platform_fee_amount_raw,
...
SELECT
    ...
    seller,
    buyer,
    amount_raw::decimal(38, 0) AS amount_raw,
    amount_original,
    amount_usd,
    currency_symbol,
    currency_contract,
