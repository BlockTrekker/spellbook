...
latest_delta as (
    ...
    from (
        select
            ...
            evt_block_time as update_time,
            row_number() over (partition by contract_address order by evt_block_number desc, tx.index desc) as `ordering`
        from {{ source('sudo_amm_ethereum','LSSVMPair_general_evt_DeltaUpdate') }}
        inner join {{ source('ethereum','transactions') }}
            on
                tx.block_time = evt.evt_block_time
                and tx.hash = evt.evt_tx_hash
...
latest_spot_price as (
    ...
    from (
        select
            ...
            evt_block_time as update_time,
            row_number() over (partition by contract_address order by evt_block_number desc, tx.index desc) as `ordering`
        from {{ source('sudo_amm_ethereum','LSSVMPair_general_evt_SpotPriceUpdate') }}
        inner join {{ source('ethereum','transactions') }}
            on
                tx.block_time = evt.evt_block_time
                and tx.hash = evt.evt_tx_hash
...
latest_settings as (
    ...
    from latest_spot_price
    full join latest_delta on t1.pool_address = t2.pool_address
    full join latest_pool_fee on t1.pool_address = t3.pool_address or t2.pool_address = t3.pool_address
),
...
{% if is_incremental() %}
    , full_settings_backfilled as (
        select * from (
            ...
            full outer join initial_settings
