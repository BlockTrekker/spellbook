    ...
    INNER JOIN
        (
            ...
            FROM
                {{ source('ethereum', 'transactions') }}
            INNER JOIN
                {{ ref('opensea_inorganic_volume_filter_wallet_funders_wallets') }}
                ON et.to = wl.wallet
            LEFT JOIN aggregators ON et.`from` = agg.aggregator_address
            LEFT JOIN cex ON et.`from` = c.cex_address
            LEFT JOIN disperse ON et.`from` = d.disperse_address
            LEFT JOIN contracts ON et.`from` = c.contract_address
            WHERE
                agg.aggregator_address IS NULL
