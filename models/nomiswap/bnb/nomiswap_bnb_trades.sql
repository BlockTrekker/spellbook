...
WITH nomiswap_dex AS (
    SELECT t.evt_block_time AS block_time,
        to                                                         AS taker,
            ...
            ''                                                           AS trace_address,
            t.evt_index
    FROM {{ source('nomiswap_bnb', 'NomiswapPair_evt_Swap') }}
    INNER JOIN {{ source('nomiswap_bnb', 'NomiswapFactory_evt_PairCreated') }}
...
LEFT JOIN {{ ref('tokens_erc20') }}
    ON
        erc20a.contract_address = nomiswap_dex.token_bought_address
        AND erc20a.blockchain = "bnb"
LEFT JOIN {{ ref('tokens_erc20') }}
    ON
        erc20b.contract_address = nomiswap_dex.token_sold_address
        AND erc20b.blockchain = "bnb"
LEFT JOIN {{ source('prices', 'usd') }}
    ON
        p_bought.minute = date_trunc("minute", nomiswap_dex.block_time)
...
LEFT JOIN {{ source('prices', 'usd') }}
    ON
        p_sold.minute = date_trunc("minute", nomiswap_dex.block_time)
