...
events_raw as (
    select
        ...
        saleid as `sale_id`
    from {{ source('zonic_optimism', 'ZonicMarketplace_evt_ZonicBasicOrderFulfilled') }}
    {% if not is_incremental() %}
...
transfers_raw as (
    ...
    from events_raw
    inner join {{ ref('transfers_optimism_eth') }}
        on
    ...
    from events_raw
    inner join {{ source('erc20_optimism','evt_transfer') }}
        on
...
from events_raw
inner join source_optimism_transactions
    on
        er.tx_hash = tx.hash
        and tx.block_number = er.block_number
left join ref_nft_aggregators
    on
        agg.contract_address = tx.to
        and agg.blockchain = "optimism"
left join ref_tokens_nft
...
left join source_prices_usd
    on
        p1.minute = date_trunc("minute", er.block_time)
        and p1.contract_address = er.currency_contract
left join {{ source('erc721_optimism','evt_transfer') }}
    on
    ...
    {% endif %}
        ...
        {% endif %}
left join {{ source('erc1155_optimism','evt_transfersingle') }}
    on
