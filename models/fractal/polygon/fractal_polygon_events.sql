...
WITH listing_detail AS (
    SELECT
        assetcontract AS nft_contract_address,
        contract_address,
        evt_block_number,
        evt_block_time,
        evt_index,
...
trades AS (
    ...
    FROM {{ source ('fractal_polygon', 'Marketplace_evt_NewSale') }}
    INNER JOIN listing_detail ON s.listingid = l.listingid
    WHERE
        1 = 1
...
trade_amount_detail AS (
    ...
    INNER JOIN trades
        ...
        {% endif %}
            ...
            {% endif %}
    WHERE
        t.original_currency IN ('0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', '0x0000000000000000000000000000000000001010')
...
LEFT JOIN tokens.erc20 ON erc.blockchain = 'polygon' AND erc.contract_address = a.currency_contract
