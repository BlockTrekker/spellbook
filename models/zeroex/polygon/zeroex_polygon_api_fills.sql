...
v4_rfq_fills_no_bridge AS (
    ...
    FROM {{ source('zeroex_polygon', 'ExchangeProxy_evt_RfqOrderFilled') }}
...
v4_limit_fills_no_bridge AS (
    ...
    FROM {{ source('zeroex_polygon', 'ExchangeProxy_evt_LimitOrderFilled') }}
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
            AND zeroex_tx.block_number = fills.evt_block_number
...
otc_fills AS (
    SELECT
        ...
        (zeroex_tx.tx_hash IS NOT NULL) AS swap_flag,
        FALSE AS `matcha_limit_order_flag`
    FROM {{ source('zeroex_polygon', 'ExchangeProxy_evt_OtcOrderFilled') }}
    INNER JOIN zeroex_tx
        ON
            zeroex_tx.tx_hash = fills.evt_tx_hash
            AND zeroex_tx.block_number = fills.evt_block_number
...
newbridgefill AS (
    ...
    WHERE
        logs.topic1 = '0xe59e71a14fe90157eedc866c4f8c767d3943d6b6b2e8cd64dddcc92ab4c55af8'
...
direct_plp AS (
    SELECT
        ...
        TRUE AS swap_flag,
        FALSE AS `matcha_limit_order_flag`
    FROM {{ source('zeroex_polygon', 'ExchangeProxy_evt_LiquidityProviderSwap') }}
    INNER JOIN zeroex_tx
        ON
...
SELECT
    ...
    CASE
        WHEN
            maker_token IN (
                '0x2791bca1f2de4661ed88a30c99a7a9449aa84174', '0x7ceb23fd6bc0add59e62ac25578270cff1b9f619', '0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270', '0xc2132d05d31c914a87c6611c10748aeb04b58e8f',
...
LEFT OUTER JOIN {{ ref('tokens_erc20') }} ON ts.contract_address = taker_token AND ts.blockchain = 'polygon'
LEFT OUTER JOIN {{ ref('tokens_erc20') }} ON ms.contract_address = maker_token AND ms.blockchain = 'polygon'
