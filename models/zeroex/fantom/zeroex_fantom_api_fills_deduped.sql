...
WITH fills_with_tx_fill_number AS (
    SELECT
...
fills_first_last AS (
    SELECT
        CASE
            ...
            ELSE 1
        END AS taker_consider_flag,--from
        CASE
            WHEN a.maker_token = b.taker_token AND a.maker_token_amount_raw = b.taker_token_amount_raw THEN 0
            ELSE 1
        END AS maker_consider_flag,
        SUM(CASE
            WHEN a.maker_token = b.taker_token AND a.maker_token_amount_raw = b.taker_token_amount_raw THEN 0
            ELSE 1
        END) OVER (PARTITION BY a.tx_hash ORDER BY a.evt_index DESC) AS hop_count,
        a.*
    FROM fills_with_tx_fill_number
    LEFT JOIN fills_with_tx_fill_number ON (a.tx_hash = b.tx_hash AND a.tx_fill_number = b.tx_fill_number - 1)
    LEFT JOIN fills_with_tx_fill_number ON (a.tx_hash = c.tx_hash AND a.tx_fill_number = c.tx_fill_number + 1)
),

deduped_bridge_fills AS (
    SELECT
        tx_hash,
...
SELECT
    ...
    a.contract_address
FROM fills_with_tx_fill_number
INNER JOIN deduped_bridge_fills
