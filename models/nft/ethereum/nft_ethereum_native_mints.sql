...
FROM {{ ref('nft_ethereum_transfers') }}
LEFT JOIN nfts_per_tx ON nft_count.tx_hash = nft_mints.tx_hash
LEFT JOIN {{ source('ethereum','traces') }}
    ON
        et.block_time = nft_mints.block_time
...
LEFT JOIN {{ source('ethereum','transactions') }}
    ON
        ...
        {% endif %}
LEFT JOIN {{ ref('nft_ethereum_aggregators') }} ON etxs.to = agg.contract_address
LEFT JOIN {{ ref('tokens_ethereum_nft') }} ON tok.contract_address = nft_mints.contract_address
LEFT JOIN namespaces ON etxs.to = ec.address
{% if is_incremental() %}
