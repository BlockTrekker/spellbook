...
FROM {{ source('erc721_ethereum','evt_transfer') }} AS t
{% if is_incremental() %}
...
FROM {{ source('erc1155_ethereum','evt_transfersingle') }} AS t
{% if is_incremental() %}
    ANTI JOIN {{this}} anti_table
            ON t.evt_tx_hash = anti_table.tx_hash
    {% endif %}
...
FROM (
    ...
    FROM {{ source('erc1155_ethereum', 'evt_transferbatch') }} AS t
    {% if is_incremental() %}
        ANTI JOIN {{this}} anti_table
            ON t.evt_tx_hash = anti_table.tx_hash
    {% endif %}
...
INNER JOIN {{ source('ethereum', 'transactions') }} AS et
    ON
        et.block_number = t.evt_block_number
        AND et.hash = t.evt_tx_hash
        {% if is_incremental() %}
            AND et.block_time >= date_trunc('day', now() - interval '1 week')
        {% endif %}
