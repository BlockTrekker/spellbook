...
left join {{ source('prices','usd') }}
    on
        ...
        {% endif %}
left join {{ source('erc20_ethereum','evt_transfer') }}
    on
        erc20s.evt_block_time = nft_mints.block_time
...
left join {{ ref('nft_optimism_aggregators') }}
    on etxs.to = agg.contract_address
left join nfts_per_tx
...
group by
    nft_mints.block_time, nft_mints.block_number, nft_mints.token_id, nft_mints.token_standard,
