...
FROM {{ source('erc721_avalanche_c','evt_transfer') }} AS t
{% if is_incremental() %}
...
FROM {{ source('erc1155_avalanche_c','evt_transfersingle') }} AS t
{% if is_incremental() %}
    ANTI JOIN {{this}} anti_table
        ON t.evt_tx_hash = anti_table.tx_hash
    {% endif %}
...
FROM (
    ...
    FROM {{ source('erc1155_avalanche_c', 'evt_transferbatch') }} AS t
    {% if is_incremental() %}
        ANTI JOIN {{this}} anti_table
            ON t.evt_tx_hash = anti_table.tx_hash
    {% endif %}
