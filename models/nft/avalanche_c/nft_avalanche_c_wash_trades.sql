...
filter_4 AS (
    ...
    FROM {{ ref('nft_trades') }} AS nftt
    INNER JOIN {{ ref('addresses_events_avalanche_c_first_funded_by') }} AS filter_funding_buyer
        ON
            ...
            {% endif %}
    INNER JOIN {{ ref('addresses_events_avalanche_c_first_funded_by') }} AS filter_funding_seller
        ON
