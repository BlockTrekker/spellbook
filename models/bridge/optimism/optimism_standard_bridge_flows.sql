...
WITH bridge_events AS (
    ...
    FROM (
        ...
        WHERE
            topic1 = '0xb0444523268717a02698be47d0803aa7468c00acbed2f8bd93a0459cde61dd89' --Deposit Finalized
        ...
        FROM {{ source ('optimism', 'logs') }}
        WHERE
            topic1 = '0x73d170910aba9e6d50b102db522b1dbcd796216f5128b445aa2135272886497e' --Withdrawal Initiated
            -- Other potentially helpful filters
            AND (topic2 IS NOT NULL) AND (topic3 IS NOT NULL) AND (topic4 IS NOT NULL) AND (data IS NOT NULL)
            {% if is_incremental() %}
                AND block_time > NOW() - interval '14 days'
    ...
    LEFT JOIN {{ ref ('ovm_optimism_cross_domain_messenger_messages') }}
        ON
            a.tf_type = m.msg_type
