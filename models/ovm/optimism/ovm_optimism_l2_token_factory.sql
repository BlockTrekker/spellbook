...
FROM (
    ...
    FROM {{ source( 'ovm_optimism', 'L2StandardTokenFactory_call_createStandardL2Token' ) }}

    INNER JOIN {{ source( 'ovm_optimism', 'L2StandardTokenFactory_evt_StandardL2TokenCreated' ) }}
        ON
    ...
    LEFT JOIN {{ ref('tokens_ethereum_erc20') }}
    ...
    FROM {{ source( 'ovm_optimism', 'OVM_L2StandardTokenFactory_call_createStandardL2Token' ) }}

    INNER JOIN {{ source( 'ovm_optimism', 'OVM_L2StandardTokenFactory_evt_StandardL2TokenCreated' ) }}
        ON
