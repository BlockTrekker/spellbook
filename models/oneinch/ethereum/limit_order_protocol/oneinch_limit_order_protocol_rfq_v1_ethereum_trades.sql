...
WITH limit_order_protocol_rfq_v1 AS (
    ...
    FROM
        {{ source('oneinch_lop_ethereum', 'LimitOrderProtocol_call_fillOrderRFQ') }}
    INNER JOIN
        {{ source('ethereum', 'traces') }}
        ON
            call.call_tx_hash = ts.tx_hash
...
SELECT
    ...
    src.evt_index
FROM limit_order_protocol_rfq_v1
