...
iv_base_pairs_priv as (
    select
        ...
        a.is_traded_nft,
        a.is_moved_nft
    from ref_nftearth_optimism_base_pairs
    where
        1 = 1
    ...
    select
        ...
        a.sender,
        case
            when b.tx_hash is not null then b.receiver
            else a.receiver
        end as `receiver`,
        a.zone,
        a.token_contract_address,
        ...
        a.is_moved_nft
    from ref_nftearth_optimism_base_pairs
    left join ref_nftearth_optimism_base_pairs
        on
            b.tx_hash = a.tx_hash
...
iv_nfts as (
    select
        ...
        a.receiver as `buyer`,
        case
            when nft_cnt > 1 then "bundle trade"
        ...
        coalesce(nft_cnt > 1, false) as `estimated_price`,
        is_private,
        sub_type,
        sub_idx
    from iv_base_pairs_priv
    left join iv_volume
        on
            b.block_date = a.block_date  -- tx_hash and evt_index is PK, but for performance, block_time is included
...
iv_trades as (
    select
        ...
        case
            when a.token_contract_address = '{{ c_native_token_address }}' then '{{ c_native_symbol }}'
            else e.symbol
        end as `token_symbol`,
        case
            when a.token_contract_address = '{{ c_native_token_address }}' then '{{ c_alternative_token_address }}'
        ...
        sub_idx
    from iv_nfts
    inner join source_optimism_transactions on t.hash = a.tx_hash
    left join ref_tokens_nft on n.contract_address = nft_contract_address
    left join ref_tokens_erc20
        on e.contract_address = case
            when a.token_contract_address = '{{ c_native_token_address }}' then '{{ c_alternative_token_address }}'
            else a.token_contract_address
        end
    left join source_prices_usd
        on p.contract_address = case
            when a.token_contract_address = '{{ c_native_token_address }}' then '{{ c_alternative_token_address }}'
            else a.token_contract_address
        end
        and p.minute = date_trunc("minute", a.block_time)
    left join ref_nft_aggregators on agg.contract_address = t.to
),
...
iv_columns as (
    ...
    -- basic info
        ...
        case when t.price_amount_raw > 0 then CAST((t.platform_fee_amount_raw / t.price_amount_raw * 100) as double) end as `platform_fee_percentage`,

        -- royalty info
        t.creator_fee_receiver_1 as `royalty_fee_receive_address`,
        CAST(t.creator_fee_amount_raw as double) as `royalty_fee_amount_raw`,
        case when t.price_amount_raw > 0 then CAST((creator_fee_amount_raw / t.price_amount_raw * 100) as double) end as `royalty_fee_percentage`,
        ...
        t.sub_type
    from iv_trades
    left join all_transfers
        on
            t.tx_hash = erc.evt_tx_hash
