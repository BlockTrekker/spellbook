...
WITH pools AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('balancer_v2_ethereum', 'WeightedPoolFactory_call_create') }}
            ON
    ...
    FROM (
        SELECT
            ...
            'WP' AS `pool_type`
        FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_ethereum', 'WeightedPoolV2Factory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    FROM (
        SELECT
            ...
            'WP2T' AS `pool_type`
        FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_ethereum', 'WeightedPool2TokensFactory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    FROM (
        SELECT
            ...
            'IP' AS `pool_type`
        FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_ethereum', 'InvestmentPoolFactory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    SELECT
        ...
        NULL AS normalized_weight,
        cc.symbol,
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'StablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'MetaStablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LBP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'LiquidityBootstrappingPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LBP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'NoProtocolFeeLiquidityBootstrappingPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'StablePhantomPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'ComposableStablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LP' AS `pool_type`
    FROM {{ source('balancer_v2_ethereum', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_ethereum', 'AaveLinearPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
...
SELECT
    ...
    CASE
        WHEN array_contains(array('SP', 'LP', 'LBP'), pool_type) THEN lower(pool_symbol)
