...
WITH pools AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('balancer_v2_polygon', 'WeightedPoolFactory_call_create') }}
            ON
    ...
    FROM (
        SELECT
            ...
            'WP' AS `pool_type`
        FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_polygon', 'WeightedPoolV2Factory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    FROM (
        SELECT
            ...
            'WP2T' AS `pool_type`
        FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_polygon', 'WeightedPool2TokensFactory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    FROM (
        SELECT
            ...
            'IP' AS `pool_type`
        FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_polygon', 'InvestmentPoolFactory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'StablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'MetaStablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LBP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'LiquidityBootstrappingPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LBP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'NoProtocolFeeLiquidityBootstrappingPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'StablePhantomPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'ComposableStablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LP' AS `pool_type`
    FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_polygon', 'AaveLinearPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
