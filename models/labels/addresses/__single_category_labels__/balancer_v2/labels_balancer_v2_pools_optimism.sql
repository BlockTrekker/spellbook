...
WITH pools AS (
    ...
    FROM (
        SELECT
            ...
            cc.symbol,
            'WP' AS `pool_type`
        FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_optimism', 'WeightedPoolFactory_call_create') }}
            ON
    ...
    FROM (
        SELECT
            ...
            'WP' AS `pool_type`
        FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_optimism', 'WeightedPoolV2Factory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    FROM (
        SELECT
            ...
            'WP2T' AS `pool_type`
        FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
        INNER JOIN {{ source('balancer_v2_optimism', 'WeightedPool2TokensFactory_call_create') }}
            ON
                c.evt_tx_hash = cc.call_tx_hash
                AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
        {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_optimism', 'StablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_optimism', 'MetaStablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LBP' AS `pool_type`
    FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_optimism', 'NoProtocolFeeLiquidityBootstrappingPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'SP' AS `pool_type`
    FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_optimism', 'ComposableStablePoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
    ...
    SELECT
        ...
        'LP' AS `pool_type`
    FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }}
    INNER JOIN {{ source('balancer_v2_optimism', 'AaveLinearPoolFactory_call_create') }}
        ON
            c.evt_tx_hash = cc.call_tx_hash
            AND SUBSTRING(c.poolid, 0, 42) = cc.output_0
    {% if is_incremental() %}
