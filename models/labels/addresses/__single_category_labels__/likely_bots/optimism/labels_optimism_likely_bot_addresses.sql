...
WITH sender_transfer_rates AS (
    ...
    SELECT
        ...
        SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ source('erc20_optimism','evt_Transfer') }} WHERE t.hash = r.evt_tx_hash AND t.block_number = r.evt_block_number) THEN 1 ELSE 0 END) AS num_erc20_tfer_txs,
        SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ ref('nft_transfers') }} WHERE t.hash = r.tx_hash AND t.block_number = r.block_number AND blockchain = 'optimism') THEN 1 ELSE 0 END) AS num_nft_tfer_txs,

        SUM(CASE
            WHEN EXISTS (SELECT 1 FROM {{ source('erc20_optimism','evt_Transfer') }} WHERE t.hash = r.evt_tx_hash AND t.block_number = r.evt_block_number) THEN 1
            WHEN EXISTS (SELECT 1 FROM {{ ref('nft_transfers') }} WHERE t.hash = r.tx_hash AND t.block_number = r.block_number AND blockchain = 'optimism') THEN 1
            ELSE 0
        END) AS num_token_tfer_txs,

        SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ ref('dex_trades') }} WHERE t.hash = r.tx_hash AND t.block_time = r.block_time AND blockchain = 'optimism') THEN 1 ELSE 0 END) AS num_dex_trade_txs,
        SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ ref('perpetual_trades') }} WHERE t.hash = r.tx_hash AND t.block_time = r.block_time AND blockchain = 'optimism') THEN 1 ELSE 0 END) AS num_perp_trade_txs,
        SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ ref('nft_trades') }} WHERE t.hash = r.tx_hash AND t.block_number = r.block_number AND blockchain = 'optimism') THEN 1 ELSE 0 END) AS `num_nft_trade_txs`
...
first_pass_throughput_filter AS (
    ...
    GROUP BY 1, 2
    HAVING
        MAX(hr_txs) >= 20 --had some high-ish frequency - gte 20 txs per hour at least once
        OR
...
bot_addresses AS (
    ...
    FROM (
        ...
        HAVING
            (MAX(wk_txs) >= 2000 AND MAX(max_hr_txs) >= 100) --frequency (gt 2k txs in one week and gt 100 txs in one hour)
