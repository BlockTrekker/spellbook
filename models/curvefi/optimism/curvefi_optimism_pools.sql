...
meta_pools AS (
    ...
    FROM
        (
            SELECT
                ...
                bp.token,
                mp.evt_block_number
            FROM {{ source('curvefi_optimism', 'PoolFactory_evt_MetaPoolDeployed') }} AS mp
            INNER JOIN base_pools AS bp
    ...
    INNER JOIN {{ source('erc20_optimism','evt_transfer') }} AS et
        ON
            et.evt_tx_hash = mps.evt_tx_hash
...
SELECT
    version,
    CAST(tokenid AS string) AS tokenid,
    token,
    pool
FROM agg_pools
UNION ALL
