...
WITH dexs AS (
    ...
    SELECT
        ...
        CASE
            WHEN
                l.topic1 = '0xd013ca23e77a65003c2c659c5442c00c805371b7fc1ebd4c206c41d1536bd90b'
    ...
    FROM {{ source('ethereum', 'logs') }} AS l
    INNER JOIN {{ ref('curvefi_ethereum_view_pools') }} AS p
        ON
    ...
    WHERE
        l.topic1 = '0xb2e76ae99761dc136e598d4a629bb347eccb9532a5f8bbd72e18467c3c34cc98' --TokenExchange
        {% if not is_incremental() %}
        AND l.block_time >= '{{ project_start_date }}'
        {% endif %}
        {% if is_incremental() %}
...
INNER JOIN {{ source('ethereum', 'transactions') }} AS tx
    ...
    {% endif %}
        ...
        {% endif %}
LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS erc20a ON erc20a.contract_address = dexs.token_bought_address
LEFT JOIN {{ ref('tokens_ethereum_erc20') }} AS erc20b ON erc20b.contract_address = dexs.token_sold_address
