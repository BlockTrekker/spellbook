...
with events_raw as (
    ...
    where
        contractaddress != lower('0xbe81eabdbd437cba43e4c1c330c63022772c2520') -- --exploit contract
        {% if is_incremental() %} 
...
final as (
    ...
    from events_raw
    inner join {{ source('optimism','transactions') }}
        on
            er.tx_hash = tx.hash
    ...
    left join {{ source('erc1155_optimism','evt_transfersingle') }}
        ...
        {% endif %}
            ...
            {% endif %}
    left join {{ source('erc20_optimism','evt_transfer') }}
        on
